...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    const videoLocalPath =(files?.videoFile?.length > 0 ) ? files.videoFile[0].path : "";
    if(!videoLocalPath){
        throw new ApiError(400, "Video file is required.")
    }

    const thumbnailLocalPath = (files?.thumbnail?.length>0) ? files.thumbnail[0].path:"";
    ...
    if(!video){
        throw new ApiError(500, "Something went wrong while publshing video", video)
    }
   console.log('video after uploading it cloudinary', video )
    return res.status(200).json({data:"Testing video upload route ", title, description})
})
